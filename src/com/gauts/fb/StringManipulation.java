...
//        System.out.println(app.validMountainArray(new int[]{3, 5, 5}));
    ...
    public boolean checkIfExist(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            if (arr[i] > 0) {
                if (numberList.indexOf(arr[i]) > -1) {
                    return true;
                } else {
                    numberList.add(2 * arr[i]);
                }
            } else {
                if (numberList.indexOf(arr[i]) > -1) {
                    return true;
    ...
    public int returnLongestSubStringLength(String s) {
        int output = 0;
        if (s != null && !"".equals(s)) {
            StringBuilder sb = new StringBuilder();
            char[] charArray = s.toCharArray();
            for (int i = 0; i < charArray.length; i++) {
                if (sb.indexOf(String.valueOf(charArray[i])) == -1) {
                    ...
                    sb = new StringBuilder(str.substring(str.indexOf(charArray[i]) + 1));
                    sb.append(charArray[i]);
                }
            }
            if (sb.length() > output) {
    ...
    public int stringToIntegerFailed(String s) {
        if (s == null || s.trim() == "" || s.length() > 200)
            return 0;
        ...
        for (int i = 0; i < charArray.length; i++) {
            ...
            if (charArray[i] == '0'
                    && (numString.length() == 0
                    || (numString.toString().length() == 1 && !Character.isDigit(numString.toString().charAt(0))))) {
                skippedzero = true;
                continue;
            }
            if (charArray[i] == '-' || charArray[i] == '+') {
                if (skippedzero || numString.length() > 0) {
                    return 0;
                } else {
        ...
        if (numString.toString().length() == 0
                || (numString.toString().length() == 1 && !Character.isDigit(numString.toString().charAt(0))))
            return 0;
        if (numString.toString().length() > 11
                || (numString.toString().length() == 12 && (numString.toString().charAt(0) == '-' || numString.toString().charAt(0) == '+'))) {
        ...
        long num = Long.valueOf(numString.toString());
    ...
    public int stringToInteger(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (Character.isDigit(c)) {
                if (c == '0' && numString.length() == 0) {
                    skippedzero = true;
                    continue;
                }
                numString.append(c);
            } else {
                if (!(skippedzero || numString.length() > 0 || sign)) {
                    if (c == ' ') {
                        continue;
                    }
                    if (c == '+') {
        ...
        if (numString.toString().length() == 0
                || (numString.toString().length() == 1 && !Character.isDigit(numString.toString().charAt(0))))
            return 0;
        if (numString.toString().length() > 10) {
            if (multiplier == -1)
                return Integer.MIN_VALUE;
            else
                return Integer.MAX_VALUE;
        ...
        long num = Long.valueOf(numString.toString()) * multiplier;
        if (num > Integer.MAX_VALUE) return Integer.MAX_VALUE;
        if (num <= Integer.MIN_VALUE) return Integer.MIN_VALUE;
        return Integer.valueOf(numString.toString()) * multiplier;
    }
    ...
    public List<List<Integer>> threeSumUnoptimized(int[] nums) {
            ...
            for (int j = i + 1; j < nums.length; j++) {
                ...
                for (int k = j + 1; k < nums.length; k++) {
                    ...
                    if (value1 + value2 + value3 == 0) {
                        ...
                        Arrays.sort(values);
                        String key = values[0] + String.valueOf(values[1]) + values[2];
    ...
    public ListNode returnNode(BigInteger total) {
        ListNode result = new ListNode();
        BigInteger[] div = total.divideAndRemainder(BigInteger.TEN);
        result.val = (int) div[1].intValue();
        total = div[0];
        ListNode next = result;
        while (total.compareTo(BigInteger.ZERO) > 0) {
            ListNode node = new ListNode();
            BigInteger[] vals = total.divideAndRemainder(BigInteger.TEN);
            node.val = (int) vals[1].intValue();
            total = vals[0];
            next.next = node;
            next = node;
        }
    ...
    public ListNode addTwoNumbers(ListNode l1, ListNode l2) {
        ...
        BigInteger valueL1 = BigInteger.ZERO;
        BigInteger valueL2 = BigInteger.ZERO;
        ListNode list1 = l1;
        ListNode list2 = l2;
        while (list1 != null || list2 != null) {
            if (list1 != null) {
                valueL1 = valueL1.add(multiplier.multiply(new BigInteger(String.valueOf(list1.val))));
                list1 = list1.next;
            }
            if (list2 != null) {
                valueL2 = valueL2.add(multiplier.multiply(new BigInteger(String.valueOf(list2.val))));
                list2 = list2.next;
            }
            multiplier = multiplier.multiply(BigInteger.TEN);
        }
        BigInteger total = valueL1.add(valueL2);
        return returnNode(total);
    }
